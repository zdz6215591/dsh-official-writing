...
function pinIssuesToDoc(doc, issues) {
  ...
  for (const issue2 of issues) {
    ...
    const full = locateInText(text, issue2);
    if (!full) continue;
    const fullRange = offsetsToRange(map, full.start, full.end, doc.content.size);
    if (!fullRange) continue;
    const visual = visualMarkRange(text, issue2);
    if (visual) {
      const tight = offsetsToRange(map, visual.start, visual.end, doc.content.size);
      if (tight && markSliceValid(doc, { ...issue2, from: tight.from, to: tight.to })) {
        next.push({ ...issue2, start: visual.start, end: visual.end, from: tight.from, to: tight.to });
        continue;
      }
    }
    next.push({ ...issue2, start: full.start, end: full.end, from: fullRange.from, to: fullRange.to });
  }
  return next;
}
