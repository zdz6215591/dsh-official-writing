...
function locateInText(text, issue) {
  ...
  if (type === "insert") {
    if (context) {
      const idx2 = indexOfPreferred(text, context, issue.start);
  ...
  const idx = indexOfPreferred(text, original, issue.start ?? (context ? text.indexOf(context) : void 0));
...
// src/host/jobs.ts
var STREAM_TIMEOUT_MS = 25e3;
var AUDIT_TIMEOUT_MS = 4e4;
...
async function streamText(ctx, options, onDelta, maxWaitForTextMs = 0) {
...
async function streamMaybeOffThinking(ctx, options, onDelta, efforts, requested, allowReasoningFallback = false, once = false) {
  const attempts = streamAttempts({ requested, efforts, preferOff: !requested && !once });
  const planned = once ? attempts.slice(0, 1) : attempts;
  const waitText = once ? 6e3 : 0;
  const call = async (next) => deadline(streamText(ctx, next, onDelta, waitText), next.signal);
  let lastError;
  let lastReasoning = "";
