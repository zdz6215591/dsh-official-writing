...
// src/host/jobs.ts
var STREAM_TIMEOUT_MS = 18e3;
var AUDIT_TIMEOUT_MS = 35e3;
...
function pickEffort(requested, efforts) {
  if (!efforts.length) return void 0;
  if (requested && efforts.some((item) => item.id === requested)) {
    return requested;
  }
  const lowest = efforts.find(
    (item) => /low|min|none|off|disable|chat|fast|minimal/i.test(`${item.id} ${item.name || ""}`)
  );
  return lowest?.id || efforts[0].id;
}
...
async function runJob(ctx, request, job) {
  ...
  try {
    const effort = request.task === "autocomplete" ? pickEffort(void 0, route.efforts) : request.task === "audit" && request.depth !== "deep" ? pickEffort(void 0, route.efforts) : pickEffort(request.effort, route.efforts);
...
${before.slice(-1200)}`)],
      ...
      job.text = (cleaned || (raw2 || job.text).replace(/\s+/g, " ").trim()).slice(0, 80);
      return;
    }
