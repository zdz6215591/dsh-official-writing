...
async function runJob(ctx, request, job) {
  ...
  try {
    const effort = request.task === "autocomplete" ? void 0 : request.task === "audit" && request.depth !== "deep" ? void 0 : pickEffort(request.effort, route.efforts);
...
${before.slice(-800)}`)],
      ...
      job.text = cleanModelText(raw2 || job.text).slice(0, 80);
      return;
    }
